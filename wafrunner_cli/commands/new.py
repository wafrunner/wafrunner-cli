...
@app.command()
def download(
    year: Optional[int] = typer.Option(None, "--year", "-y", help="The year of the CVEs to download."),
    all_time: bool = typer.Option(False, "--all-time", "-a", help="Download CVEs from 1999 to the current year."),
    latest: bool = typer.Option(False, "--latest", "-l", help="Download the latest CVEs since the last update."),
    ...
    """Download CVE data from the NIST NVD API."""
    modes = [year is not None, all_time, latest]
    if sum(modes) > 1:
        print("[bold red]Error:[/bold red] --year, --all-time, and --latest are mutually exclusive.")
        raise typer.Exit(code=1)
    if sum(modes) == 0:
        print("[bold red]Error:[/bold red] Please specify a download mode: --year, --all-time, or --latest.")
        raise typer.Exit(code=1)
    ...
    if latest:
        handle_latest_download(output_dir)
    else:
        handle_archive_download(year, all_time, output_dir, update)


def handle_latest_download(output_dir: Path):
    """Handles downloading the latest CVEs since the last update."""
    print(f"Checking for latest CVEs to save in [green]{output_dir}[/green]...")
    last_mod_date = get_last_modified_date(output_dir)

    if last_mod_date is None:
        print("[bold red]Error:[/bold red] No existing CVE data found. Cannot determine where to start fetching updates.")
        print("Please run a full download using --all-time or --year first.")
        raise typer.Exit(code=1)

    start_date = last_mod_date
    end_date = datetime.now(timezone.utc)

    print(f"Fetching updates from [bold cyan]{start_date.isoformat()}[/bold cyan] to [bold cyan]{end_date.isoformat()}[/bold cyan].")

    all_new_vulns = []
    start_index = 0
    total_results = 1  # Dummy value to enter the loop

    with httpx.Client(base_url=NIST_API_BASE_URL, timeout=60.0, follow_redirects=True) as client:
        # TODO: Add API key support from config
        with Progress(SpinnerColumn(), TextColumn("[progress.description]{task.description}"), BarColumn(), TextColumn("[progress.percentage]{task.percentage:>3.0f}%"), TextColumn("({task.completed} of {task.total})"), TimeRemainingColumn(), transient=False) as progress:
            task = progress.add_task("Downloading latest CVEs...", total=None)
            while start_index < total_results:
                ...
                try:
                    ...
                    # --- FIXED BLOCK ---
                    # Check if the task's total has been set. This happens on the first API call.
                    if progress.tasks[task].total is None:
                        total_results = data.get('totalResults', 0)
                        if total_results == 0:
                            print("\n[bold green]✔ No new or updated CVEs found.[/bold green]")
                            progress.update(task, total=0) # Update progress to show 0/0
                            break # Exit the loop, the function will finish cleanly
                        progress.update(task, total=total_results)
                    # --- END FIXED BLOCK ---

                    new_vulns = data.get('vulnerabilities', [])
                    all_new_vulns.extend(new_vulns)
                    progress.update(task, advance=len(new_vulns))
                    
                    start_index += data.get('resultsPerPage', len(new_vulns))

                    if start_index < total_results:
                        time.sleep(REQUEST_DELAY_SECONDS)

                except httpx.HTTPStatusError as e:
                    print(f"[bold red]Error fetching data from NIST API: {e.response.status_code} - {e.response.text}[/bold red]")
                    raise typer.Exit(code=1)
                except (json.JSONDecodeError, KeyError) as e:
                    print(f"[bold red]Error parsing API response: {e}[/bold red]")
                    raise typer.Exit(code=1)

    if all_new_vulns:
        timestamp = datetime.now().strftime('%Y%m%d-%H%M%S')
        output_filename = output_dir / f"nvd-cves-update-{timestamp}.json"
        
        final_data = {
            "resultsPerPage": len(all_new_vulns),
            "startIndex": 0,
            "totalResults": len(all_new_vulns),
            "format": "NVD_CVE",
            "version": "2.0",
            "timestamp": datetime.now(timezone.utc).isoformat(),
            "vulnerabilities": all_new_vulns
        }
        
        with output_filename.open('w', encoding='utf-8') as f:
            json.dump(final_data, f, indent=2)
        print(f"\n[bold green]✔ Successfully saved {len(all_new_vulns)} new/updated CVEs to {output_filename.name}.[/bold green]")
    elif total_results > 0:
         print("\n[bold yellow]⚠ Download finished, but no new CVEs were saved.[/bold yellow]")
    # If total_results was 0, the message is handled inside the loop.

def get_last_modified_date(cve_dir: Path) -> Optional[datetime]:
    """Scans all CVE JSON files to find the most recent lastModified date."""
    latest_date = None
    json_files = sorted(list(cve_dir.glob('nvd-cves-*.json')), key=os.path.getmtime, reverse=True)

    if not json_files:
        return None

    with Progress(TextColumn("{task.description}"), BarColumn(), TextColumn("{task.completed}/{task.total} files")) as progress:
        task = progress.add_task("Scanning for last update...", total=len(json_files))
        for file_path in json_files:
            try:
                ...
                for vuln in data.get('vulnerabilities', []):
                    mod_str = vuln.get('cve', {}).get('lastModified')
                    if mod_str:
                        # Using fromisoformat and ensuring the result is offset-naive for comparison
                        mod_date = datetime.fromisoformat(mod_str).replace(tzinfo=None)
                        if latest_date is None or mod_date > latest_date:
                            latest_date = mod_date
            except (json.JSONDecodeError, IOError):
                # Ignore corrupted, empty, or unreadable files
                pass
            finally:
                progress.update(task, advance=1)
    
    # Return the date as an offset-aware UTC datetime object
    return latest_date.replace(tzinfo=timezone.utc) if latest_date else None


def handle_archive_download(year: Optional[int], all_time: bool, output_dir: Path, update: bool):
    """Handles downloading CVEs for a specific year or all time."""
