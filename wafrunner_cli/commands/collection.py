...
def create_collection(
    ...
    identifier: Optional[List[str]] = typer.Option(None, "--id", "-i", help="A CVE ID or VulnID to add. Can be used multiple times."),
    ...
    if not any([identifier, file]):
        print("[bold red]Error:[/bold red] You must provide at least one source: --id or --file.")
        raise typer.Exit(code=1)
        
    collection_file = COLLECTIONS_DIR / f"{name}.json"
    if collection_file.exists():
        print(f"[bold red]Error:[/bold red] Collection '{name}' already exists. Use 'collection delete' to remove it first.")
        raise typer.Exit(code=1)
    ...
    if identifier: input_ids.update(identifier)
    ...
    for item_id in sorted(list(input_ids)):
        ...
        if resolved_ids:
            ...
            print(f"[bold yellow]Warning:[/bold yellow] Could not resolve identifier: {item_id}")
